%#######################################################################
%
%                 * MRI Eroded Meniscus FIT Program *
%
...
%     mri_em_fit.xlsx, in the "Results" directory.
%
%          Reads regions of interests that are eroded by one pixel
%     inferiorly and superiorly to avoid including femoral or tibial
%     cartilage and any synovial fluid.  See seg_em_rois.m.
%
...
%             names must contain "emroi".  See rd_m_dicom.m and
%             seg_em_rois.m.
%
...
%
resdir = fullfile('Eroded_Results');   % Results directory
%
ifirst = true;          % First write to file
xlsnam = 'mri_em_fit.xlsx';            % Results spreadsheet
...
hdrs2 = {'Pixels' 'T1R/T2S' 'RSS' 'ValidPix' 'Mean' 'Min' 'Max' ...
         'SD' 'COV'};
%
psnam = fullfile(resdir,'mri_em_fit_');          % Start of PS file name
...
%
   ...
   subj = eval(subjnam(1:3));          % Subject number
%
...
%
   ...
   roinams = {d.name}';
   idr = contains(roinams,'emroi','IgnoreCase',true);      % Masks
%
...
%
      idm = contains(rhonams,roinam(1:end-11));  % Get matching file
      if ~any(idm)
        error([' *** ERROR in mri_em_fit:  Matching T1rho MAT', ...
...
% Plot Results
%
      sid = ['Subject ' subjnam ', ' ltxt ' Leg, T1\rho', ...
             'Eroded ROIs'];
...
%
      ...
      load(fullfile(sdir,roinam),'bbox','cmprt','maskmal','maskmam', ...
           'maskmpl','maskmpm','rsll','rslm');
%
      idm = contains(starnams,roinam(1:end-11)); % Get matching file
      if ~any(idm)
        error([' *** ERROR in mri_em_fit:  Matching T2* MAT', ...
               ' file not found for ROI MAT file:  ' roinam '!']);
      end
...
% Plot Results
%
      sid = ['Subject ' subjnam ', ' ltxt ' Leg, T2*', 'Eroded ROIs'];
...
% Save to MAT File
%
save(fullfile(resdir,'mri_em_fit.mat'),'t1r_res','t1r_npx', ...
